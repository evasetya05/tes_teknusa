...
from teknusa.utils import get_current_site
from teknusa.cache_utils import cache_decorator, cache
...
class BaseModel(models.Model):
    ...
    def save(self, *args, **kwargs):
        is_update_views = self.__class__.__name__ == 'Job' and 'update_fields' in kwargs and kwargs['update_fields'] == ['views']
        if is_update_views:
            self.__class__.objects.filter(pk=self.pk).update(views=self.views)
        else:
...
class Job(BaseModel):
    ...
    def next_job(self):
        # Next
        return Job.objects.filter(id__gt=self.id, status='p').order_by('id').first()

    @cache_decorator(expiration=60 * 100)
    def prev_job(self):
        # previous
        return Job.objects.filter(id__lt=self.id, status='p').first()
